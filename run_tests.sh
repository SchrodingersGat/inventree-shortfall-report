#!/usr/bin/env bash
# Run the component_shortfall test suite against a local InvenTree dev instance.
#
...
#   ./run_tests.sh component_shortfall.tests.test_shortfall.OutstandingPartsTests
#   ./run_tests.sh component_shortfall.tests -v 2              # extra manage.py test args
set -euo pipefail
...
TEST_TARGET="${1:-component_shortfall.tests}"
if [ "$#" -gt 0 ]; then
    shift
fi
...
INVENTREE_PLUGINS_MANDATORY=component-shortfall \
